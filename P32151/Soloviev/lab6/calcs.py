...
def rungeKutt(func, x_0, y_0, h, b, e):
    ...
    while abs(y_current_h_div_2 - y_current) > e:
        ...
        td = [h, y_current, y_current_h_div_2]
        table.add_row(td)
        print(table)
        h /= 2
...
def rungeKutt_method(func, a, b, x_0, y_0, h, e):
...
def adams_method(func, a, b, x_0, y_0, h, e):
    ans = adams(func, x_0, y_0, h, int((b - x_0) / h))
    return ans


def euler_next(func, x_prev,x, y_prev, h):
    return y_prev + h/2*(func(x_prev,y_prev)+func(x,y_prev+h*func(x_prev,y_prev)))


def eulers_method_v2(func, b, x_0, y_0, h, e):
    answer_x = []
    answer_y = []
    answer_x.append(x_0)
    answer_y.append(y_0)
    ...
    while x_prev<b:
        x = x_prev + h
        y = euler_next(func, x_prev, x, y_prev, h)
        answer_y.append(round(y, int(abs(math.log10(e)))))
        answer_x.append(x)
        x_prev = x
        y_prev = y
    return list(zip(answer_x, answer_y))
...
def lyutiy_eulers_method(func, a, b ,x_0, y_0, h, e):
    ans = eulers_method_v2(func, b, x_0, y_0, h, e)
    return ans
