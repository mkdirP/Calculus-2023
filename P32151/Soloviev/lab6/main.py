...
def main():
    ...
    while True:
        ...
        if x_0 <= b:
                    ...
                    for i in range(len(ansR)):
                        for j in range(i, len(ansE)):
                            if (ansE[j][0] >= ansR[i][0]):
                                ...
                                table.add_row(td)
                                # x_cor.append(ansR[i][0])
                                # y_cor.append(y_correct)
                                break
                else:
                    for i in range(len(ansR)):
                    ...
                    for i in range(len(ansA)):
                        for j in range(i, len(ansR)):
                            if (ansR[j][0] >= ansA[i][0]):
                                ...
                                break
            print(table)
            # corr = list(zip(x_cor, y_cor))
            wait = input("Press enter to continue")
            print("\033[32m{}".format("Метод Рунге-Кута:"))
            ...
            print(table)
            wait = input("Press enter to continue")
            print("\033[32m{}".format("Метод Адамса:"))
            th = ["x", "y_a"]
            table = PrettyTable(th)
            print("\033[0m".format(), end='')
            for i in range(len(ansA)):
            ...
            print(table)
            wait = input("Press enter to continue")
            print("\033[32m{}".format("Усовершенствованный метод Эйлера:"))
            print("\033[0m".format(), end='')
            th = ["x", "y_r"]
            table = PrettyTable(th)
            for i in range(len(ansE)):
                ...
                table.add_row(td)
            print(table)
            wait = input("Press enter to show graph (u have no choice)")
