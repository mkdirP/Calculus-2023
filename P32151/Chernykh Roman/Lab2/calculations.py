...
# Метод хорд
def chord_method(function, aa, bb, error):
    a = aa
    b = bb
    x = b
    iterations = 0
    while True:
        ...
        if function(a) * function(x0) > 0:
            ...
            b = x0
        iterations += 1
        if (abs(x - x0) <= error) or (abs(a - b) <= error) or (abs(function(x)) <= error):
...
def iterrations_method(function, a, b, error):
    ...
    while (abs(x_prev - x_current) > error) or (abs(function(x_current)) > error):
        ...
        print("X = " + str(x_current) + "; X(i-1) = " + str(x_prev) + "; f(x) = " + str(function(x_current)) + ";")
        iterations += 1
        if iterations > 1000:
