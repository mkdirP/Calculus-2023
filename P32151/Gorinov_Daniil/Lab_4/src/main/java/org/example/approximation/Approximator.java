...
public class Approximator {
    ...
    public ApproximationResult cubicApproximation(double[][] functionTable) {
        ...
        for (double[] doubles : functionTable) {
            x_sum += doubles[0];
            x2_sum += Math.pow(doubles[0], 2);
            x3_sum += Math.pow(doubles[0], 3);
            x4_sum += Math.pow(doubles[0], 4);
            x5_sum += Math.pow(doubles[0], 5);
            x6_sum += Math.pow(doubles[0], 6);
            y_sum += doubles[1];
            xy_sum += doubles[0] * doubles[1];
            x2y_sum += Math.pow(doubles[0], 2) * doubles[1];
            x3y_sum += Math.pow(doubles[0], 3) * doubles[1];
        }
